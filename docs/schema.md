...
（着手時に本ファイルへ追記する）。優先順位・依存関係でのスコープ分解（A〜F）は
[roadmap.md](./roadmap.md)のPhase3セクション参照。集計対象からカスタム種目を除く方針は当面維持
（詳細は[backlog.md](./backlog.md)参照）。
...
## Phase4の検討結果（2026-09-08）

Phase4着手前の設計セッションで詰めた論点。画面単位の詳細設計自体は各Issue着手時に行うが、
スキーマ・スコープ分割に直結する決定はここに残す。

- **記録の公開範囲とマルチグループ**：`workouts`にグループ紐付けカラムは追加しない。投稿者が
  複数グループに所属している場合、その記録は所属する**全グループのメンバーに自動公開**される
  （グループごとに公開先を選ぶ機能は持たない）。「所属していれば自動公開」というconcept.mdの
  既存方針をマルチグループの場合にも素直に適用した形
- **通知の配信方式**：Phase4初弾ではリアルタイム配信（Socket.io）を採用せず、**画面遷移／
  ページ読み込み時にAPIを叩いて未読件数・通知一覧を取得するだけ**の方式にする（定期ポーリングの
  タイマーも設けない）。少人数クローズドグループの「いいね・コメント」中心のアプリでは反映の
  多少の遅れは実害が小さいと判断。技術スタック欄の「Socket.io（Phase4で採用検討）」はこの判断で
  一旦見送りとし、「バッジの更新が遅い」等の不満が実際に出たタイミングで定期ポーリング→
  Socket.ioの順に段階的に強化する
- **ランキング画面**：グループごとの専用画面として新設する（統計画面とは別）。将来複数指標を
  切り替えられるレイアウト（タブ／セレクタ）を想定して作るが、**Phase4初弾の中身は「合計挙上重量」
  1指標のみ**実装する（自重種目は0kg扱い）。期間は**週間（日曜始まり〜土曜、Phase3の週定義と統一）／
  月間（1日〜月末）／通算（全期間）**の3タブ。ランキング集計対象は公式種目のみという既存方針は継続
- **Phase4初弾のスコープ**：グループ基盤（作成・招待コード発行／再発行・招待コードでの参加・
  メンバー一覧・退会・削除〈オーナーのみ〉）に限定する。reactions/comments/notifications/rankingは
  別Issueに分離し、1 Issue = 1セッション方針に沿って積み上げる
...
- **種目マスタの削除は、カスタム種目（`created_by`が自分）に限りソフトデリートで可能**（[Issue #113](https://github.com/ConniConni/torebu/issues/113)で対応、方針転換前は「削除機能自体を作らない」としていた）。公式種目には引き続き削除機能を作らない。物理削除だと過去の`workout_sets`/`routine_exercises`のFKが壊れるため、`exercises.deleted_at`を立てるだけに留める。削除済みでも`GET /exercises`のレスポンスからは除外しない（過去記録の種目名解決に使われ続けるため）。新規の記録・ルーティンへの追加候補からの除外、および新規使用のブロック（`400 invalid_exercise`）はアプリ側で行う
...
- **`birth_date`/`gender`/`occupation`は登録時に必須で入力させる**（2026-09-09方針変更、Issue #158）。
  ...
  方針だったが、リリース前整理で変更した。3項目とも「回答しない」を選べる必須の選択式にすることで、
  スキップによる未入力を防ぎつつプライバシーにも配慮している。用途自体はPhase3の属性別分析
  （年代・性別・職業別のクロス集計）のまま変わらない
  - `birth_date`は年月のみ入力させ、DBの`Date`型へは日を1日固定で保存する（例：2000年5月→`2000-05-01`）
  - `gender`は既存の`Gender` enumをそのまま使う
  - `occupation`は自由記述`String?`から`Occupation` enum（`student`/`company_employee`/`self_employed`/`executive`/`homemaker`/`other`/
    `no_answer`）に変更した（マイグレーション：`20260909210000_add_occupation_enum`）
  - 実装・画面の詳細は[spec.md](./spec.md)の「①新規登録の追加項目」参照
  - 既存ユーザー（3項目とも未入力）への入力促し方は別途検討（`docs/backlog.md`参照）
- **外部キーは基本的にインデックス化する**（[Issue #108](https://github.com/ConniConni/torebu/issues/108)）。
  Postgresは他のDBと違い外部キーを自動でインデックス化しないため、`@@index`を明示的に付ける方針とする。
  例外は「カーディナリティが極端に低い」「書き込みが支配的でそのカラムでの検索がほぼ無い」
  「すでに複合indexの先頭カラムに含まれている」場合のみ。`workouts`は`user_id`単独ではなく
  `(user_id, performed_at)`の複合indexにして単独indexを代替している（一覧・カレンダー・
  日付引き当てが常に両方の条件で絞り込むため）
