...
- **`gender`/`occupation`も`birth_date`と同じくPhase3の属性別分析用**。初期スキーマ設計時に
  「年代だけでなく性別・職業でもクロス集計できたら面白そう」という発想で`birth_date`と一緒に
  追加したが、用途の明記自体を当時忘れていた（2026-09-08判明・追記）。`birth_date`と同様、
  登録時は任意のままPhase3で入力を促す想定。現状は登録画面に入力欄が無くAPI・DBカラムのみ存在する
  未使用状態（Phase3着手時にUI・APIを実装する）
