...
- `middleware/auth.ts`の`return navigateTo('/login')`を一時的にコメントアウトして保存し、ログアウトした状態で`/routines`を開いてみる → リダイレクトされずにページ自体は表示されてしまうはずだ(ただし中のAPI呼び出しは`backend`側の`requireAuth`で401になるため、データ表示は壊れる)。フロント側のガードとバックエンド側の`requireAuth`(具体例1参照)は別々の仕組みで、どちらか一方だけでは不十分なことが体感できる。試したら元に戻すこと
