...
- **いいねユーザーの表示**：`GET /groups/:id/workouts`のレスポンス各要素に`reactorNames`
  （いいねした人の表示名の配列、いいねした順）を追加した。フロント（グループの記録フィード）は
  いいねボタンの下に「Aさん、Bさんがいいねしました」のように表示する（3人以上は「Aさん、Bさん他N人」
  と省略）
  - 実装のついでに、従来`reactionCount`用に発行していた`groupBy`クエリを廃止し、`reactorNames`と
    同じ`findMany`（displayName込み）から件数・自分のいいね有無も導出する形に整理した
    （クエリ数は変わらない）
  - `POST/DELETE /workouts/:id/reactions`のレスポンスは変更していない（`reactionCount`/`reactedByMe`
    のまま）。フロントは自分がいいね・いいね取り消しした直後の`reactorNames`表示を、レスポンスに
    含まれない分だけ画面側で楽観的に反映している（同じ表示名の他メンバーがいたときの取り違えは
    未対応の既知の制約。表示名の重複自体を防ぐ仕組みが無いため）
