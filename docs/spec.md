...
② ホーム（/）
 ...
 └─「ルーティン」─────> ⑤ 一覧（/routines）          │
                            ...
                            └─> ⑤ 編集（/routines/[id]）
                                  ...
                                  削除も🗑️ボタン→画面内2段階確認（2026-09-25、Issue #261で
                                  確認なし即削除から変更）。最後の1件まで削除すると、その種目
                                  自体もルーティンから削除される
   ...
   │      明示的な「保存」ボタンは無い。「削除」も既存どおり利用可能（🗑️ボタン→画面内2段階確認。
   │      2026-09-25、Issue #261で確認なし即削除から変更）。重量欄には`placeholder="自重"`
