...
| `/`（ログイン中） | ② | ホーム（カレンダー・期間別サマリー（今週/今月/通算）・記録カードの本体削除・通知バッジ） | `GET /workouts`, `GET /workouts/:id`, `DELETE /workouts/:id`, `GET /stats/volume`, `GET /notifications/unread-count`, `POST /auth/logout` | なし（ページ内で分岐、下記参照） |
...
- トップ画面は[WelcomeScreen.vue](../frontend/app/components/WelcomeScreen.vue)。静的なイラスト
  （`frontend/app/assets/images/top_image.jpeg`）を画面いっぱいに`object-cover`で敷き、「ログイン」
  「新規登録」ボタンはイラスト下部の無地オレンジの余白部分に重ねて配置する（ボタンをイラストの下に
  別領域として置く案・縦積み案も検討したが、イラストを画面いっぱいに見せたいという要望に沿って
  オーバーレイ方式にした）。余白部分の位置はPythonでピクセル解析し、画像下端から約9%
  （y座標91%〜100%）が無地であることを確認したうえでボタンの絶対配置位置を決めた。
...
- **左右の見切れは解消済み**：初回導入時の`top_image.png`（縦横比2:3、1024×1536）はスマホ画面
  （概ね0.42〜0.52）と縦横比の差が大きく、`object-cover`で左右が大きく見切れていた。2026-09-09に
  縦横比0.562（940×1672）の`top_image.jpeg`へ差し替え、さらに構図（人物・テキスト・吹き出しを
  中央寄りに配置）も調整したことで、375〜430px幅で見切れが無いことを確認した
...
- 当初は連続記録日数（ストリーク）を名前欄の隣にテキストで表示する案で実装したが、「1年後にその
  ...
  （下記参照。表示場所・見た目は変わったが、計算ロジック自体は変わっていない）

**期間別サマリーカード（今週/今月/通算、Phase3-D → Issue #169で統合）の実装メモ**
- ②ホーム画面の「＋今日の記録をつける」ボタン・「ルーティン一覧」リンクとカレンダーの間に、
  白背景のカード2枚を横並びで表示する。左カードが期間別サマリー（今週/今月/通算をタブで
  切り替え、選んだ期間の合計負荷重量とトレ日数を縦積み表示）、右カードが「週別推移」として
  直近4週間の合計負荷重量を横棒グラフで表示する（今週の行を一番上、値ラベルは出さずバーの
  長さのみで比較させる形）。タブの初期選択は「今週」
...
- 合計負荷重量の単位は**t（トン）表記**（例:「0.8t」）。当初はkg表記だったが、今月/通算のような
  大きい値だと桁が増えて見づらいという判断でtに統一した（今週のサマリーも含め、画面内で単位を
...
- **動物換算キャプション**：「0.8t」という数字だけだと実感が湧きにくいという指摘を受け、
  合計負荷重量を海の生き物の体重に例えるキャプション（例:「シャチ×1.4」）を、数字の下の元の
  「合計負荷重量」ラベルの代わりに表示する（0kg以下でキャプションを出せないときだけ従来どおり
  「合計負荷重量」ラベルを表示する）。参照動物はマンボウ(約1t)・シャチ(約5.5t)・
  クジラ(約110t、正式にはシロナガスクジラ)の3種（陸の動物も比較検討したが、「海の動物だけに
  揃える」という指示を受けて海の生き物のみにした。中間の種は当初サメ(ジンベエザメ)だったが、
  絵柄が伝わりにくいという指摘を受けてシャチに変更した、2026-09-11）
  ...
  - 倍数の表記は「約◯匹(頭)分」から「×◯」（小数第1位で四捨五入、整数のときは小数点を付けない）
    に短縮した。短縮した分の余白でアイコンを大きくしている（クジラの視認性が悪いという指摘）
...
- 新規バックエンドAPIは作らず、既存`GET /stats/volume`（`range=all`。今月/通算の合計負荷重量も
  同じレスポンスから計算するため`1m`ではなく`all`を使う）のレスポンスをフロントで週/月/全期間に
  集計する。トレ日数は`allRecordedDates`（`GET /workouts`の`performedAt`一覧）を流用する
  （「フロント集計のみで完結させる」方針を踏襲）
- 週の定義は日曜始まり〜土曜（[HomeCalendar.vue](../frontend/app/components/HomeCalendar.vue)の
  曜日表示と揃える）。今週/今月/通算の集計ロジックはそれぞれ
  [weeklySummary.ts](../frontend/app/utils/weeklySummary.ts)の`sumWeeklyVolume()`等、
  [trainingVolume.ts](../frontend/app/utils/trainingVolume.ts)の`sumVolumeInMonth()`・
  `sumTotalVolume()`、[trainingDays.ts](../frontend/app/utils/trainingDays.ts)（すべて
  Vitestでテスト済み）。週別推移（`weeklyVolumeTrend()`）はデータが無い週も0kgとして返す
  （0埋めしない`/stats/volume`本体とは異なり、4週分を常に揃えて描画するため）
