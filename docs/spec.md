...
- トップ画面は[WelcomeScreen.vue](../frontend/app/components/WelcomeScreen.vue)。静的なイラスト
  ...
  オーバーレイ方式にした）。余白部分の位置はPythonでピクセル解析し、画像下端から約11.6%
  （y座標88.4%〜100%）が無地であることを確認したうえでボタンの絶対配置位置を決めた。
...
- **既知の制約（縮小済み）**：初回導入時の`top_image.png`（縦横比2:3、1024×1536）はスマホ画面
  （概ね0.42〜0.52）と縦横比の差が大きく、`object-cover`で左右が大きく見切れていた。2026-09-09に
  縦横比0.562（940×1672）の`top_image.jpeg`へ差し替え、見切れをかなり軽減した（375〜430px幅で
  確認、右端の吹き出しは見切れなくなった。左端の文字がわずかに見切れる程度は残る）。
  縦横比のミスマッチ自体は解消していないため、完全に見切れを無くすにはスマホ画面により近い
  縦横比（9:16〜9:19程度）のイラストが必要（詳細は`docs/backlog.md`「判断保留」節参照）
