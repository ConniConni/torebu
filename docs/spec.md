...
② ホーム（/）
 ...
 └─「ルーティン」─────> ⑤ 一覧（/routines）          │
   ...
   │           ・⑤で目安セット（重量・回数）を設定していても、ここではまだ使われない。
   │             「入力待ちの種目」に積まれるのはexerciseIdのみで、値は引き続き毎回手入力する
   │             （目安セットをセット入力へ引き継ぐのは次のIssueで対応予定。`docs/backlog.md`参照）
   │           ・⑤が0件なら、⑤への案内リンクが出る
   │
   ├─ セット入力（重量・回数）→「記録」を押すたびに1件ずつ保存される
   │      ・重量欄は次のセットのためにあえてクリアしない（同じ重量が続くことが多いため）
   │      ・「この種目の入力を終える」で入力欄を閉じる
   │      ・注意：この入力欄だけは他（メモ・セット編集）と違い自動保存されない。「記録」を
   │        押さないまま「ホームへ戻る」で離脱すると入力値は消える（`docs/backlog.md`参照）
   │
   ├─ 記録済みセットの「編集」→ 重量・回数の入力欄からフォーカスが外れる（blur）たびに
   │      自動保存される（`PATCH /workouts/:id/sets/:setId`）。明示的な「保存」ボタンは無く、
   │      「閉じる」で表示モードへ戻る。「削除」も既存どおり利用可能
   │
   ├─ 記録済み種目カードの「＋セット追加」→ ④種目選択を経由せず、その場でその種目の
   │      セット入力欄を開く（「＋種目を追加」から同じ種目を選び直す手間を省く）。
   │      入力欄を開いている間はボタン自体を非表示にする
   │
