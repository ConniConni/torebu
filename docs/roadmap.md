...
- **「イチオシこだわり共有」は今回のPhase4では着手せず、今後の拡張候補として保留する**
  （2026-09-23決定）。グループ基盤・記録フィード・いいね・コメント・通知・ランキングでPhase4の
  中核（交流とランキング）は一通り揃ったと判断し、優先度が低い機能として区切りをつけた。設計方針
  自体は`docs/concept.md`・`docs/schema.md`「Phase4の検討結果」に記載済みのため、着手する際は
  そちらを起点に個別Issue化する

## 次のIssue候補（Phase4後・Phase5前）

- ~~**通知の種類の拡張**~~（2026-09-24設計決定。詳細は[backlog.md](./backlog.md)「通知の種類の拡張」節）
  → 3つのIssueに分けて対応済み（2026-09-24）
  - [x] 通知一覧の汎用化・5分遅延・共通判定＋新メンバー参加の通知
        （[Issue #249](https://github.com/ConniConni/torebu/issues/249)）
  - [x] 自己ベスト更新の通知（`payload`追加・本人へのその場の表示を含む）
        （[Issue #253](https://github.com/ConniConni/torebu/issues/253)）
  - [x] 通算の節目・久しぶりの復帰の通知（トレ日数表示を「セットがある日」に揃える変更を含む）
        （[Issue #255](https://github.com/ConniConni/torebu/issues/255)）

- ~~**収益シミュレーション**~~：2026-09-11の検討で試算・価格決定まで完了済み。
  **グループPro：月額800円／年額8,000円／買い切り20,000円**（人数上限拡張のみ）、
  **個人テーマ：300円/買い切り**で確定（本番実績ゼロの仮説価格、公開後に実データで調整する前提）。
  詳細は[backlog.md](./backlog.md)「判断保留」節「収益シミュレーション」参照。実装（Stripe連携等）は
  「収益化：課金の仕組み・不足機能の洗い出し」節（同ファイル）を参照し、着手時にIssue化する

- **本番リリース（Vercelへのデプロイ）**（2026-09-12方針決定）：ホスティング先をVercelに決定した。
  Expressのサーバーレス関数対応・`trust proxy`設定・環境変数設定等の実装が必要。詳細は
  [backlog.md](./backlog.md)「本番ホスティングの方針」参照

- ~~**各画面の「ホームへ戻る」の位置・文言を統一する**~~ →
      [Issue #155](https://github.com/ConniConni/torebu/issues/155)で対応済み（2026-09-09）。
      ③記録本体画面（`workouts/new.vue`）のヘッダーを、左に`← ホームに戻る`のボタン、右に`<h1>`
      （日付＋タイトル）という⑤ルーティン・⑧統計と同じ配置・文言に揃えた。詳細は
      [backlog.md](./backlog.md)「Phase4前のUI整理（Issue #127）で見つかった課題」参照
