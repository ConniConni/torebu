...
- [x] **記録へのいいね**：対象は`workout`のみ（[Issue #140](https://github.com/ConniConni/torebu/issues/140)で
      対応済み、2026-09-09）
- **次点候補：コメント**：いいねと同じ`workout`単位・同じ認可ロジック（所属グループで同席している
      メンバーの記録のみ）を再利用できるため、次に着手するなら有力（`docs/backlog.md`
      「Phase4の進捗・未着手項目」参照）
...
- Phase2（部位ハイライト可視化）は完了済み。Phase4はグループ基盤・記録フィード・いいねまで
  着手済み。残るコメント・通知・ランキング・イチオシこだわり共有は「Phase4（交流とランキング）」
  セクション参照
