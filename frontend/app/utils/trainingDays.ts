...
//
// 「記録がある日」は`hasSets`が1件以上ある日のみを数える（メモのみの日は含まない）。呼び出し側
// （HomeScreen.vue・mypage/index.vue）が渡すrecordedDatesの時点でhasSetsによる絞り込みを済ませる。
// 通知の通算の節目（C1）・久しぶりの復帰（C2）の判定と数字を揃えるための方針（Issue #255。
// 以前はhasSetsを問わずメモのみの日も含めていた）
